...
const String photoA = 'TT_20260504_211530_0a1b.jpg';
const String photoB = 'TT_20260504_214000_0c2d.jpg';
...
Transect buildTransect() {
  final first = TestRecord(
      species: 'Parus major', note: 'pevanje', photos: [photoA, photoB]);
  final second = TestRecord(species: 'Sitta europaea');
...
void main() {
  ...
  setUpAll(() => TrackerConfig.current = testConfig());

  setUp(() async {
    root = await Directory.systemTemp.createTemp('tracker_media_test');
  ...
  test('KMZ embeds the record photos and survives a round trip', () async {
    ...
    final back = restored.markers!.single.records!;
  ...
  test('backup archive carries the records JSON and the photos', () async {
    ...
    final backup = outPath('$TrackerConfig.current.backupBaseName.zip');
  ...
  test('a KMZ is recognised by its bytes, not by its name', () async {
    ...
    expect(imported.markers!.single.records!.first.species, 'Parus major');
  });
  ...
  test('both export formats import back to the same records', () async {
    ...
    for (final imported in [fromKml, fromKmz]) {
      ...
      final record = marker.records!.first as TestRecord;
    ...
    expect(fromBare.markers!.single.records!.first.photos, [photoA, photoB]);
    expect(MediaService().exists(photoA), isFalse);
  });
  ...
  test('a legacy point without a photos key restores as an empty list', () {
    ...
    for (final record in json['species'] as List<dynamic>) {
      (record as Map).remove('photos');
    }

    for (final record in Placemark.fromJson(json).records!) {
