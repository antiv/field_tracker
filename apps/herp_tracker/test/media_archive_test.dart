...
import 'package:herp_tracker/utils/backup_utils.dart';
import 'package:herp_tracker/utils/file_utils.dart';
import 'package:herp_tracker/utils/kml_utils.dart';
import 'package:herp_tracker/utils/kmz_utils.dart';

const String photoA = 'HT_20260504_211530_0a1b.jpg';
const String photoB = 'HT_20260504_214000_0c2d.jpg';
...
Transect buildTransect() {
  ...
  final second = Species()
    ...
    ..observedAt = DateTime(2026, 5, 4, 21, 40)
    ..count = 1;
...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('herp_media_test');
  ...
  test('a transect with no photos still exports as plain KML', () {
    ...
    expect(transect.toKML(), contains('Bufo bufo'));
  });
  ...
  test('a KMZ is recognised by its bytes, not by its name', () async {
    ...
    expect(imported.markers!.single.species!.first.species, 'Bufo bufo');
  });
  ...
  test('both export formats import back to the same records', () async {
    ...
    for (final imported in [fromKml, fromKmz]) {
      ...
      expect(record.species, 'Bufo bufo');
